...
class SourceOutcome:
    ...
    credential_exposures: tuple[ResultObservation, ...] = ()
    ...
    for item in values:
        if kind == 'hostname':
            ...
            try:
                ...
                continue
        elif kind in {'credential-exposure', 'infostealer', 'person'}:
...
async def _collect_observations(
    ...
    credential_exposures: set[ResultObservation],
) -> None:
    ...
    for route in ResultRoute:
        if route not in source_spec.routes or (route is ResultRoute.SUBDOMAINS and not request.include_hostnames):
            continue
        getter_name, kind = _ROUTE_GETTERS[route]
    ...
    if request.source == 'builtwith':
        ...
        if getter := getattr(adapter, 'get_credential_exposures', None):
            credential_exposures.update(_normalize_values(request, 'credential-exposure', await getter()))
...
def _source_outcome(
    ...
    return SourceOutcome(
        ...
        tuple(sorted(credential_exposures)),
    )
...
async def run_source(
    ...
    *,
    ...
    credential_exposures: set[ResultObservation] = set()
    ...
    try:
        ...
        result_count = len(observations) + len(credential_exposures)
    ...
    return _source_outcome(request, execution, observations, asn_attributions, credential_exposures)
