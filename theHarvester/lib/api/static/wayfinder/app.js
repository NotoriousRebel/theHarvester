(() => {
  ...
  const state = {
    ...
    route: null,
    theme: localStorage.getItem('wayfinder-theme') || 'system',
    pollTimer: null,
  ...
  function applyTheme() {
    ...
    localStorage.setItem('wayfinder-theme', state.theme);
  }
  ...
  async function loadWorkspace() {
    ...
    const [sourcesResponse, runsResponse] = await Promise.all([
      api('/api/wayfinder/sources'), api('/api/wayfinder/runs')
    ]);
  ...
  async function selectRun(runId, options = {}) {
    ...
    renderHistory();
    try {
      const response = await api(`/api/wayfinder/runs/${encodeURIComponent(runId)}`);
  ...
  function renderAuthorization(run) {
    ...
    const options = [
      ...
      ['Discovery source proxies', request.proxies ? 'Selected' : 'Off'],
      ['DNS lookup', request.dns_lookup ? 'Selected' : 'Off'],
      ...
      ['Takeover transport', request.take_over ? 'Direct, DNS-pinned' : 'Off'],
  ...
  function mountResultTable(rows) {
    ...
    state.resultTable = new Tabulator(nodes.resultWorkbench.querySelector('#result-grid'), {
      ...
      columns: [
        ...
        {title: 'Sources', field: 'sources', formatter: cell => `<span class="source-cell">${escapeHtml((cell.getValue() || []).join(', ') || 'Not recorded')}</span>`, minWidth: 170, widthGrow: 1, responsive: 2, headerFilter: 'input', headerFilterFunc: columnTextFilter, headerFilterPlaceholder: 'Filter sources'},
      ]
    });
  ...
  async function refreshSelected() {
    ...
    try {
      const [detailResponse, runsResponse] = await Promise.all([
        api(`/api/wayfinder/runs/${encodeURIComponent(selectedId)}`), api('/api/wayfinder/runs')
      ]);
  ...
  async function submitRun(event) {
    ...
    try {
      const response = await api('/api/wayfinder/runs', {
        method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload)
      });
      const run = await response.json();
      closeDialog(nodes.newRunDialog);
      state.selectedId = run.run_id;
      const runsResponse = await api('/api/wayfinder/runs');
      state.runs = await runsResponse.json();
      renderHistory();
  ...
  async function submitImport(event) {
    ...
    try {
      const response = await api(`/api/wayfinder/import?filename=${encodeURIComponent(file.name)}`, {
  ...
  async function requestCancellation() {
    ...
    try {
      const response = await api(`/api/wayfinder/runs/${encodeURIComponent(selectedId)}/cancel`, {method: 'POST'});
      const detail = await response.json();
      if (state.selectedId !== selectedId) return;
      state.detail = detail;
      renderDetail();
  ...
  async function downloadServerExport(format) {
    try {
      const response = await api(`/api/wayfinder/runs/${encodeURIComponent(state.selectedId)}/export.${format}`);
      const disposition = response.headers.get('Content-Disposition') || '';
      const match = disposition.match(/filename="([^"]+)"/);
      downloadBlob(await response.blob(), match?.[1] || `wayfinder-results.${format}`);
    } catch (error) {
  ...
  function exportRouteCsv() {
    const rows = [['type', 'value', 'dns_status', 'sources'], ...routeRows().map(result => [
      result.type, result.value, result.dns_status || '', (result.sources || []).join(',')
    ])];
  ...
  nodes.resultSearch.addEventListener('input', event => {
    ...
    state.resultTable?.setFilter(row => !query || row.value.toLowerCase().includes(query) || (row.sources || []).join(' ').toLowerCase().includes(query));
  });
