...
WAYFINDER_SESSION_CONTEXT = b'theharvester-wayfinder-local-session'
...
def wayfinder_session_token(configured_api_key: str) -> str:
    return hmac.digest(configured_api_key.encode(), WAYFINDER_SESSION_CONTEXT, 'sha256').hex()
...
def get_api_key(
    ...
    expected_cookie = wayfinder_session_token(configured_api_key)
