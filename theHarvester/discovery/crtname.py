...
_ICANN_TLD_EXTRACT = tldextract.TLDExtract(cache_dir=None, suffix_list_urls=())


class SearchCrtName:
    """Collect hostname candidates from crt.name's composite passive index."""
    ...
    def __init__(self, word: str) -> None:
        ...
        extracted = _TLD_EXTRACT(self.word) if self.word else None
        self.apex = extracted.top_domain_under_public_suffix if extracted else ''
        if extracted and not self.apex:
            icann_extracted = _ICANN_TLD_EXTRACT(self.word)
            self.apex = icann_extracted.top_domain_under_public_suffix
            if not self.apex and not extracted.suffix and not icann_extracted.suffix:
                self.apex = self.word
        if not self.apex:
            self.word = ''
        ...
        async with AsyncFetcher.stream_records(
            ...
            async for record in response:
                ...
                if not self._valid_hostname(candidate):
                    malformed = True
                    continue
                if normalize_scoped_hostname(candidate, self.apex) is None:
                    malformed = True
                    continue
                normalized = normalize_scoped_hostname(candidate, self.word)
                if normalized is not None and normalized != self.word:
