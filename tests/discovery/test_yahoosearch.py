...
@pytest.mark.asyncio
async def test_yahoo_contract_uses_exact_pages_and_normalizes_evidence(
...
@pytest.mark.asyncio
async def test_yahoo_empty_responses_return_no_evidence(monkeypatch: pytest.MonkeyPatch) -> None:
    async def fake_fetch_all(
        urls: list[str] | set[str],
        **_kwargs: Any,
    ) -> list[str]:
