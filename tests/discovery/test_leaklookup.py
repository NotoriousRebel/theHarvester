...
async def test_leaklookup_emails_and_breaches_reach_completed_result_and_jsonl(monkeypatch, tmp_path: Path) -> None:
    ...
    assert {'type': 'breach', 'value': 'Example Breach', 'sources': ['leaklookup'], 'actions': []} in records
    assert {'type': 'email', 'value': 'alice@example.com', 'sources': ['leaklookup'], 'actions': []} in records
