...
async def test_verified_domain_results_reach_completed_jsonl_and_sqlite_handoff(
    ...
    assert {'type': 'breach', 'value': 'ExampleBreach', 'sources': ['hibpverified'], 'actions': []} in records
    assert {'type': 'email', 'value': 'alice@example.com', 'sources': ['hibpverified'], 'actions': []} in records
