...
@pytest.mark.asyncio
async def test_crt_name_queries_the_registrable_apex_but_keeps_the_requested_scope(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[dict[str, Any]] = []
    ...
    async def stream_records(url: str, **kwargs: Any) -> AsyncIterator[FakeResponse]:
        calls.append({'url': url, **kwargs})
        yield FakeResponse(('www.example.com', 'deep.www.example.com', 'outside.example.com'))
    ...
    assert calls[0]['params'] == {'apex': 'example.com'}
...
@pytest.mark.asyncio
async def test_crt_name_falls_back_to_a_valid_scope_with_an_unknown_suffix(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[dict[str, Any]] = []
...
def test_crt_name_uses_the_bundled_private_suffix_list_without_network_updates() -> None:
    assert crtname._TLD_EXTRACT.suffix_list_urls == ()
    assert crtname._TLD_EXTRACT.include_psl_private_domains is True


@pytest.mark.asyncio
async def test_crt_name_keeps_a_private_suffix_tenant_as_the_provider_apex(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[dict[str, Any]] = []

    def private_extract(_target: str) -> SimpleNamespace:
        return SimpleNamespace(
            top_domain_under_public_suffix='tenant.private.example.com',
            suffix='private.example.com',
        )

    def unexpected_icann_extract(_target: str) -> None:
        raise AssertionError('a complete private-suffix extraction must not fall back to the broader ICANN apex')
    ...
    monkeypatch.setattr(crtname, '_TLD_EXTRACT', private_extract)
    monkeypatch.setattr(crtname, '_ICANN_TLD_EXTRACT', unexpected_icann_extract)
    monkeypatch.setattr(crtname.AsyncFetcher, 'stream_records', stream_records)

    await crtname.SearchCrtName('tenant.private.example.com').process()

    assert calls[0]['params'] == {'apex': 'tenant.private.example.com'}


@pytest.mark.asyncio
async def test_crt_name_falls_back_to_icann_apex_for_a_private_suffix_root(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[dict[str, Any]] = []

    def private_extract(_target: str) -> SimpleNamespace:
        return SimpleNamespace(top_domain_under_public_suffix='', suffix='private.example.com')

    def icann_extract(_target: str) -> SimpleNamespace:
        return SimpleNamespace(top_domain_under_public_suffix='example.com', suffix='com')

    @contextlib.asynccontextmanager
    async def stream_records(url: str, **kwargs: Any) -> AsyncIterator[FakeResponse]:
        calls.append({'url': url, **kwargs})
        yield FakeResponse(('deep.private.example.com', 'sibling.example.com'))

    monkeypatch.setattr(crtname, '_TLD_EXTRACT', private_extract)
    monkeypatch.setattr(crtname, '_ICANN_TLD_EXTRACT', icann_extract)
    monkeypatch.setattr(crtname.AsyncFetcher, 'stream_records', stream_records)
    search = crtname.SearchCrtName('private.example.com')

    await search.process()

    assert calls[0]['params'] == {'apex': 'example.com'}
    assert await search.get_hostnames() == {'deep.private.example.com'}
    assert search.execution_status == 'completed'


@pytest.mark.asyncio
async def test_crt_name_rejects_a_bare_known_public_suffix(monkeypatch: pytest.MonkeyPatch) -> None:
    def suffix_extract(_target: str) -> SimpleNamespace:
        return SimpleNamespace(top_domain_under_public_suffix='', suffix='public.example')

    def unexpected_request(*_args: Any, **_kwargs: Any) -> None:
        raise AssertionError('a bare public suffix must not reach crt.name')

    monkeypatch.setattr(crtname, '_TLD_EXTRACT', suffix_extract)
    monkeypatch.setattr(crtname, '_ICANN_TLD_EXTRACT', suffix_extract)
    monkeypatch.setattr(crtname.AsyncFetcher, 'stream_records', unexpected_request)
    search = crtname.SearchCrtName('public.example')
