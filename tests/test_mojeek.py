...
class TestMojeekSearch:
    ...
    async def test_keyless_mode_uses_scraping_without_json(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert set(await search.get_hostnames()) == {'docs.example.com', 'example.com'}
    ...
    async def test_keyed_api_success_parses_results_without_scraping(
        ...
        assert set(await search.get_hostnames()) == {
            'api.example.com',
            'blog.example.com',
            'example.com',
        }
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [
                {'results': [{'url': 'https://search.example.com', 'title': 'admin@example.com'}]},
                '',
            ],
            [
                {'results': [{'url': 'https://search.example.com', 'title': 'admin@example.com'}]},
