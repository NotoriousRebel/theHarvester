    ...
    def start(self) -> None:
        ...
        while time.monotonic() < deadline:
            if self._process.poll() is not None:
                self.stop()
                pytest.fail(f'Wayfinder test server exited during startup; see {self.server_log}')
            try:
                ...
                time.sleep(0.05)
        self.stop()
        pytest.fail(f'Wayfinder test server did not become ready; see {self.server_log}')
...
class BrowserFailures:
    ...
    def _record_response(self, response: Response) -> None:
        ...
        if response.status >= 500 or (path.startswith('/api/wayfinder/') and response.status == 401):
...
@pytest.fixture
def wayfinder_server(tmp_path: Path, unused_tcp_port: int) -> WayfinderServer:
    repo_root = Path(__file__).parents[2]
    artifact_dir = repo_root / 'test-results'
    artifact_dir.mkdir(exist_ok=True)
    server_log = artifact_dir / f'wayfinder-server-{unused_tcp_port}.log'
    environment = os.environ.copy()
    environment.update(
        {
            'THEHARVESTER_API_KEY': 'wayfinder-e2e-key',
            'THEHARVESTER_WAYFINDER_ARTIFACTS': str(tmp_path / 'artifacts'),
            'THEHARVESTER_WAYFINDER_DB': str(tmp_path / 'wayfinder.sqlite'),
            'THEHARVESTER_WAYFINDER_WORKER': 'disabled',
    ...
    server = WayfinderServer(repo_root, unused_tcp_port, environment, server_log)
    server.start()
    try:
        yield server
    finally:
...
@pytest.fixture
def wayfinder_server_url(wayfinder_server: WayfinderServer) -> str:
    return wayfinder_server.url
