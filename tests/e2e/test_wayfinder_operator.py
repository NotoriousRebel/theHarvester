...
pytestmark = pytest.mark.wayfinder_e2e


def record_wayfinder_api_statuses(page: Page, server_url: str) -> dict[str, int]:
    api_statuses: dict[str, int] = {}

    def record_response(response) -> None:
        if response.url.startswith(f'{server_url}/api/wayfinder/'):
...
def test_fresh_browser_session_authenticates_through_operator_page(
    wayfinder_server_url: str,
    page: Page,
) -> None:
    api_statuses = record_wayfinder_api_statuses(page, wayfinder_server_url)
    page.goto(f'{wayfinder_server_url}/')

    expect(page.get_by_role('heading', name='No enumeration runs yet')).to_be_visible()
    assert api_statuses['/api/wayfinder/sources'] == 200
    assert api_statuses['/api/wayfinder/runs'] == 200
    expect(page.get_by_text('Invalid API key')).to_have_count(0)


def test_operator_page_replaces_a_legacy_root_cookie(
    wayfinder_server_url: str,
    page: Page,
) -> None:
    ...
    assert api_statuses['/api/wayfinder/runs'] == 200
    expect(page.get_by_text('Invalid API key')).to_have_count(0)
    assert {cookie['path'] for cookie in page.context.cookies() if cookie['name'] == 'theharvester-api-key'} == {'/api/wayfinder'}
...
def test_browser_session_stays_authenticated_after_server_restart(
    ...
    page.goto(f'{wayfinder_server.url}/')
    expect(page.get_by_role('heading', name='No enumeration runs yet')).to_be_visible()

    wayfinder_server.restart()
    page.reload()
...
def test_versioned_assets_and_tooltips_work_at_supported_viewports(
    ...
    stylesheet = page.locator('link[href*="/static/wayfinder/app.css"]')
    script = page.locator('script[src*="/static/wayfinder/app.js"]')
...
def test_disabled_worker_rejects_submission_without_creating_a_run(
    ...
    browser_failures.allow_response('POST', 503, '/api/wayfinder/runs')
    ...
    with page.expect_response(
        lambda response: response.url == f'{wayfinder_server_url}/api/wayfinder/runs' and response.request.method == 'POST'
    ) as submission:
        page.locator('#submit-run-button').click()

    assert submission.value.status == 503
    expect(page.locator('#new-run-error')).to_have_text('Wayfinder execution worker is disabled')
    history = page.context.request.get(f'{wayfinder_server_url}/api/wayfinder/runs')
    assert history.status == 200
    assert history.json() == []


def test_operator_can_submit_overridable_execution_controls(
    wayfinder_server_url: str,
    page: Page,
    browser_failures,
) -> None:
    ...
    page.route(f'{wayfinder_server_url}/api/wayfinder/runs', capture_submission)
    page.goto(f'{wayfinder_server_url}/')
...
def test_operator_can_select_sources_by_result_capability(wayfinder_server_url: str, page: Page) -> None:
    page.goto(f'{wayfinder_server_url}/')
    catalog = page.context.request.get(f'{wayfinder_server_url}/api/wayfinder/sources').json()
...
def test_polling_recovers_after_one_transient_refresh_failure(
    ...
    browser_failures.allow_response('GET', 503, '/api/wayfinder/runs/retry-run')
    browser_failures.allow_console_error(
        'Failed to load resource: the server responded with a status of 503 (Service Unavailable)'
    )
    run = {
    ...
    page.route(f'{wayfinder_server_url}/api/wayfinder/runs', route_runs)
    page.route(f'{wayfinder_server_url}/api/wayfinder/runs/retry-run', route_detail)
    page.goto(f'{wayfinder_server_url}/')
...
def test_workspace_startup_failure_has_an_inline_retry(
    ...
    browser_failures.allow_response('GET', 503, '/api/wayfinder/sources')
    browser_failures.allow_console_error(
        'Failed to load resource: the server responded with a status of 503 (Service Unavailable)'
    )
    source_calls = 0
    ...
    page.route(f'{wayfinder_server_url}/api/wayfinder/sources', fail_once)
    page.goto(f'{wayfinder_server_url}/')
...
def test_unchanged_poll_keeps_result_table_filters(wayfinder_server_url: str, page: Page) -> None:
    run = {
        'run_id': 'stable-run',
        'target': 'example.com',
        'status': 'running',
        ...
        'source_executions': [],
        'results': [{'type': 'subdomain', 'value': 'api.example.com', 'sources': ['crtsh']}],
        'screenshots': [],
        'log': '',
        'error': None,
    }

    page.route(f'{wayfinder_server_url}/api/wayfinder/runs', lambda route: route.fulfill(json=[run]))
    page.route(f'{wayfinder_server_url}/api/wayfinder/runs/stable-run', lambda route: route.fulfill(json=run))
    page.goto(f'{wayfinder_server_url}/')
    expect(page.locator('#request-options')).to_contain_text('Discovery source proxiesSelected')
    expect(page.locator('#request-options')).to_contain_text('Takeover transportDirect, DNS-pinned')
...
def test_submission_notification_matches_the_loaded_lifecycle(
    ...
    page.route(f'{wayfinder_server_url}/api/wayfinder/runs', route_runs)
    page.route(f'{wayfinder_server_url}/api/wayfinder/runs/toast-run', route_detail)
    page.get_by_role('button', name='Start enumeration').first.click()
    page.locator('#run-target').fill('example.com')
...
def test_completed_empty_import_explains_terminal_outcome(
    ...
    evidence_file.write_text(
        json.dumps(
            {
                'run_id': 'wayfinder-e2e-empty-run',
                'target': 'example.com',
...
def test_operator_can_import_and_analyze_fixture_evidence_through_the_real_ui(
    wayfinder_server_url: str,
    page: Page,
    tmp_path: Path,
) -> None:
    page.set_viewport_size({'width': 1440, 'height': 900})
    page.context.grant_permissions(['clipboard-read', 'clipboard-write'], origin=wayfinder_server_url)
    page.goto(f'{wayfinder_server_url}/')
    catalog = page.context.request.get(f'{wayfinder_server_url}/api/wayfinder/sources').json()
    ...
    evidence = {
        'run_id': 'wayfinder-e2e-broad-run',
        'target': 'example.com',
        ...
        'results': [
            {'type': result_type, 'value': f'{result_type}.example.com', 'sources': [ordered_sources[0]['name']]}
        ...
        + [
            {
                ...
                'dns_status': 'resolved',
                'sources': [ordered_sources[1]['name']],
            }
        ],
    }
    ...
    source_filter = page.locator('.tabulator-col[tabulator-field="sources"] .tabulator-header-filter input')
    ...
    expect(source_filter).to_have_attribute('placeholder', 'Filter sources')
    ...
    expect(page.locator('.tabulator-row:visible')).to_have_count(2)
    source_filter.press_sequentially(ordered_sources[0]['name'])
    expect(page.locator('.tabulator-row:visible')).to_have_count(1)
    expect(page.locator('.tabulator-row:visible')).to_contain_text('ip.example.com')
    source_filter.press('ControlOrMeta+a')
    source_filter.press('Backspace')
    expect(page.locator('.tabulator-row:visible')).to_have_count(2)

    resize_handles = page.locator('.tabulator-header .tabulator-col-resize-handle')
    expect(resize_handles).to_have_count(3)
    ...
    assert 'ip.example.com' in route_csv
    ...
    assert {'type': 'ip', 'value': 'ip.example.com', 'sources': [ordered_sources[0]['name']]} in exported_json['results']
    ...
    assert 'ip.example.com' in exported_csv
