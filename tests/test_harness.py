...
def test_external_network_is_blocked_by_default() -> None:
    ...
    with socket.socket() as tcp_socket, pytest.raises(AssertionError, match='External network access is disabled'):
        tcp_socket.connect_ex(('192.0.2.1', 443))
    with (
        socket.socket(type=socket.SOCK_DGRAM) as udp_socket,
        pytest.raises(AssertionError, match='External network access is disabled'),
    ):
...
def test_loopback_network_remains_available(tmp_path: Path) -> None:
    ...
    if hasattr(socket, 'AF_UNIX'):
        with socket.socket(socket.AF_UNIX) as unix_client, pytest.raises(OSError):
