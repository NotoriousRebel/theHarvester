...
async def test_runner_normalizes_only_declared_apis_guru_routes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert outcome.credential_exposures == ()
...
@pytest.mark.asyncio
async def test_runner_separates_hudson_rock_credential_exposures(monkeypatch: pytest.MonkeyPatch) -> None:
    exposure = {
    ...
    assert outcome.credential_exposures == (
        ResultObservation(
            'hudsonrock',
            'credential-exposure',
            '{"exposure_category":"employee","provider":"hudsonrock-v3","url":"https://portal.example.test"}',
        ),
    )
    assert outcome.execution.result_count == 5
