...
- Added authenticated HarvestView bulk scheduling with timezone-aware once, hourly, daily, weekly, and monthly recurrences, editable future settings, upcoming-occurrence previews, durable per-target dispatch reservations, pause/resume and run-now controls, skip-or-queue overlap policy, and a private SQLite control plane that submits ordinary finite runs without entering portable evidence exports.
