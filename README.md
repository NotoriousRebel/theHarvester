...
Discovery uses six source workers by default. Set `-j` / `--source-workers` to a positive count; the effective count is
clamped to selected sources and changes concurrency only, never which sources or results complete. REST `source_workers`
and HarvestView's advanced control use the same setting. See [the benchmark record](docs/benchmarks/source-workers.md).
...
Hostname resolution deduplicates normalized candidates from every selected source before one scan-wide A, AAAA, and CNAME phase. It runs at most 20 hostname jobs concurrently, issues at most 3,000 record queries, and stops after 60 seconds while retaining completed evidence. Reverse DNS uses a separate scan-wide job set: overlapping `/24` ranges are deduplicated, with at most 20 active PTR jobs, 3,000 requests, and 60 seconds. Budget or runtime stops are reported as partial when completed evidence exists.
